...
class JSONPostRouterTests: XCTestCase {
    ...
    func testJSONPostStringError() {
        ...
        let session = RequestKitURLTestSession(expectedURL: "https://example.com/some_route?access_token=1234", expectedHTTPMethod: "POST", response: errorString, statusCode: 401)
        TestInterface().postJSON(session) { response in
            switch response {
            case .Success:
                XCTAssert(false, "should not retrieve a succesful response")
